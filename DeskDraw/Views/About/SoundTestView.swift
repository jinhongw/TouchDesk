...
//  DeskDraw
//
//  Created by jinhong on 2025/1/10.
//
...
//          if let testView = attachments.entity(for: "testView") {
//            testView.name = "testView"
////            testView.setOrientation(.init(angle: -.pi / 2, axis: .init(x: 1, y: 0, z: 0)), relativeTo: nil)
...
//            testView.setOrientation(.init(angle: -.pi / 2, axis: .init(x: 1, y: 0, z: 0)), relativeTo: nil)
      ...
      Attachment(id: "testView") {
        Button {
          if let sound = Int(systemSounds[1]) {
            AudioServicesPlaySystemSound(SystemSoundID(sound))
          }
        } label: {
          Label("1", systemImage: "speaker.wave.1.fill")
        }
        .withCustomButtonStyle()
      }
    }
...
//            testView.setOrientation(.init(angle: -.pi / 2, axis: .init(x: 1, y: 0, z: 0)), relativeTo: nil)
        ...
        Attachment(id: "testView") {
          Button {
            if let sound = Int(systemSounds[2]) {
              AudioServicesPlaySystemSound(SystemSoundID(sound))
            }
          } label: {
            Label("2", systemImage: "speaker.wave.1.fill")
          }
          .withCustomButtonStyle()
        }
      }
    ...
    .overlay {
      Button {
        if let sound = Int(systemSounds[3]) {
          AudioServicesPlaySystemSound(SystemSoundID(sound))
        }
      } label: {
        Label("3", systemImage: "speaker.wave.1.fill")
      }
      .withCustomButtonStyle()
    }
    .overlay {
...
//            testView.setOrientation(.init(angle: -.pi / 2, axis: .init(x: 1, y: 0, z: 0)), relativeTo: nil)
        ...
        Attachment(id: "testView") {
          Button {
            if let sound = Int(systemSounds[4]) {
              AudioServicesPlaySystemSound(SystemSoundID(sound))
            }
          } label: {
            Label("4", systemImage: "speaker.wave.1.fill")
          }
          .withCustomButtonStyle()
        }
      }
    ...
    .overlay {
      Button {
        if let sound = Int(systemSounds[5]) {
          AudioServicesPlaySystemSound(SystemSoundID(sound))
        }
      } label: {
        Label("5", systemImage: "speaker.wave.1.fill")
      }
      .withCustomButtonStyle()
    }
  }
